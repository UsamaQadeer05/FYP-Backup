...
public class SymptomFragment extends Fragment {
    ...
    private void initView() {
        recyclerView = view.findViewById(R.id.rvSymptom);
        aes = new AES();
    }
...
//        age.setText(String.valueOf(a));
    ...
    //  API Call to get All Symptom through AFNL
    private void getAllSymptom() {
        allSymptomArrayList = new ArrayList<>();
        ...
        AndroidNetworking.get("http://10.0.2.2/PHP_FYP_API/api/Symptoms/Symptoms")
                ...
                .getAsObjectList(AllSymptom.class, new ParsedRequestListener<List<AllSymptom>>() {
                    @Override
                    public void onResponse(List<AllSymptom> allDiseases) {
                        // do anything with response
                        for (AllSymptom disease : allDiseases) {
                            allSymptomArrayList.add(disease);
                        }
                    }
    ...
    private void getUserSymptom() {
        ...
        AndroidNetworking.get("http://10.0.2.2/PHP_FYP_API/api/Symptoms/GetUserSymptoms/{id}")
                .addPathParameter("id", "" + id)
                .addQueryParameter("limit", "3")
                .setTag(this)
                .setPriority(Priority.LOW)
                .build()
                .getAsObjectList(Symptom.class, new ParsedRequestListener<List<Symptom>>() {
                    @Override
                    public void onResponse(List<Symptom> diseases) {
                        // do anything with response
//                        Log.d(TAG, "userList size : " + users.size());
                        for (Symptom disease : diseases) {
                            symptomArrayList.add(disease);
                        }
                        SymptomAdapter adapter = new SymptomAdapter(allDiseaseArrayList,
                                allSymptomArrayList, symptomArrayList);
